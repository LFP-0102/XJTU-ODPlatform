...
def evaluate_model(*, config: Any, data_yaml: Path, run: RunContext,
                   ...
                   write_report: bool = True) -> EvalResult:
    """评估单个模型, 生成评估报告.
    ...
    if write_report:
        ...
        (run.run_dir / "report.md").write_text(report.render_markdown(),
                                                encoding="utf-8")
...
def compare_models(*, config: Any, data_yaml: Path, run: RunContext,
                   ...
                   write_report: bool = True) -> ComparisonResult:
    ...
    if write_report:
        ...
        (run.run_dir / "comparison.md").write_text(report.render_markdown(),
                                                    encoding="utf-8")
