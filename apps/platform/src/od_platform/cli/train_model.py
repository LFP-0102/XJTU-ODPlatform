#!/usr/bin/env python
# @File       : train_model.py
# @Path       : apps/platform/src/od_platform/cli/train_model.py
# @Author     : 刘赋平
# @Date       : 2026-07-19 13:40:55
# @Version    : v1.0.0
# @Description: 
#   请在此处填写该模块的功能概述。
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--no-archive", action="store_true", help="不归档运行配置文件")
...
def main() -> int:
    ...
    yaml_path = config_ref if "/" in config_ref or "\\" in config_ref else RUNTIME_CONFIG_DIR / (config_ref if config_ref.endswith(".yaml") else f"{config_ref}.yaml")
