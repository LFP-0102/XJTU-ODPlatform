...
def _print_plan(
    ...
    logger.info(
        format_table_row(
            [f"【 总计 】", f"{str(total_files)}", _format_size(total_bytes)],
            widths,
            align,
        )
    )
...
def _delete_one(
    ...
    if size > 1024 ** 3:
        ...
        logger.info(
            f"[{idx} / {total}] 删除{rel} ({size_str}, {file_count}个文件)"
        )
...
def _execute_delete(deletable: list[tuple[Path, int, int]]) -> None:
    ...
    if failed:
        logger.warning(
            f"完成:成功{len(success)}，失败{len(failed)}，共{total}个目录"
        )
...
def _audit_context() -> dict:
    """收集下审计上下文: 主要作用出事的时候，确定谁，在那个版本的代码量，跑的"""
    try:
...
def reset_project(yes: bool = False, force: bool = False, dry_run: bool = False) -> int:
    ...
    # 审计内容:
    ctx = _audit_context()
    logger.info(f"审计: user={ctx['user']}, pid={ctx['pid']}, git_rev={ctx['git_rev']}")
    logger.info(f"审计: cwd={ctx['cwd']}")
    logger.info(f"审计: argv={" ".join(ctx['argv'])}")

    if dry_run and yes:
        logger.warning(f"!! 同时给了 --dry-run 和 --yes, 以 --dry-run模式运行" )
        yes = False
